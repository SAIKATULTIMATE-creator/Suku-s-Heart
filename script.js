...
// >>> Loading my local file <<<
loader.load( 'tree.obj', object => {
  
  // --- UPDATED FOR SINGLE OBJECT ---
  
  // 1. Define Material (PINK)
  let matPink = new THREE.MeshBasicMaterial({color: 0xff2266, wireframe: false, transparent: true, opacity: 0.75});

  // 2. Apply to the first (and only) part
  if(object.children[0]) object.children[0].material = matPink;

  // 3. Sampler (Particles on this single object)
...
var r = 6.0; // radius
var MAX_POINTS = 90000; //total particles
var pointsCount = 0;
let points = []; //3
let delay = [];  //1
let speed = [];  //2
...
ctx.fillText("💔", cnvs.width * 0.5, cnvs.height * 0.5);
...
gl_PointSize = size * ( cond ? 0.75 : ((1. - hRatio) * (smoothstep(0., 0.01, hRatio) * 0.25) + 0.75));
    gl_PointSize = mix(gl_PointSize, size * 1.5, isEffect);
    `
  );
  shader.fragmentShader = `
  uniform float time;
...
` + shader.fragmentShader;
  ...
  shader.fragmentShader = shader.fragmentShader.replace(
      ...
      `
        ...
        // Apply Heart Color
        diffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.95, 0, 0.45), vIsEffect);
        
        // Opacity
        float backgroundAlpha = 0.2; // 0.0 = invisible, 1.0 = solid
        diffuseColor.a = mix(backgroundAlpha, 1.0, vIsEffect);
`
    );
}
